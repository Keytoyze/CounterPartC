...
overwrite_ast_h = False
overwrite_ast_impl = True
overwrite_ast_ir = False
overwrite_yacc = False
...
def write_ast(ast, d):
    ...
    if overwrite_ast_h or not os.path.exists(base_ast_dir + file_name):
        with open(base_ast_dir + file_name, "w") as f:
            f.write(content)
    
    n = base_ast_dir + file_name.replace(".h", ".cpp")
    if overwrite_ast_impl or not os.path.exists(n):
        with open(n, "w") as f:
...
if overwrite_ast_h or not os.path.exists("../src/ast/terminal/token.h"):
...
if overwrite_ast_h or not os.path.exists("../src/ast/ast.h"):
...
if overwrite_yacc or not os.path.exists("../src/cpc.y"):
...
def write_ast_impl(ast, d):
    ...
    if overwrite_ast_ir or not os.path.exists(base_ir_dir + file_name):
